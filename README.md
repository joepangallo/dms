...
tools/                       maintenance scripts (Node, no dependencies to install)
  ...
  sync-sql-comments.py       check (or --apply) that content/*.json SQL matches db.html
  gen-module-labs.py         regenerate labs/module4 or labs/module5 from db.html
...
`labs/module4/` and `labs/module5/` are generated by `tools/gen-module-labs.py` from `db.html` and
the module's `sqlChecks`, so regenerate rather than hand-editing when a module changes. The `00-*.sql`
setup files are the seeds from `db.html` and are not regenerated.

The SQL in `content/*.json` and in `db.html` is kept in step, comments included — a re-render must
not silently drop the line-by-line commentary on the statements. Two blocks are known to differ for
reasons that predate this: a Module 4 calculated-column query that exists as a code block in the JSON
but only as a sandbox on the page, and a Module 8 `DELIMITER $$` that the page writes as `$`.
