...
tools/                       maintenance scripts (Node, no dependencies to install)
  ...
  gen-module5-labs.py        regenerate labs/module5 from db.html
...
rather than hand-editing when the module changes. `labs/module5/` is generated by
`tools/gen-module5-labs.py`, which reads `db.html` directly; the JSON and the page carry the same
statements and the same commentary, so either source works.
